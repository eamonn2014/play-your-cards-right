...
numericInput(inputId=("Decksize"),  
            # label = c("No of cards in each suit in deck"), 
              ...
              label = tags$span(style="color:black;font-weight:bold;font-size: 12px;", 
               
               "No of cards in each suit in deck") ,
...
[Play Your Card Right](https://en.wikipedia.org/wiki/Play_Your_Cards_Right "")

[Question](https://www.significancemagazine.com/culture/706-probability-puzzle-higher-or-lower"")

[Partial solution](https://rss.onlinelibrary.wiley.com/doi/epdf/10.1111/1740-9713.01564 "")
...
the game continues. Guess wrong, or turn over the same value card of a different suit and the game is over. ") ,
...
To get started, in a standard pack is dealt a Jack (in our app a Jack is represented by card value 10) we should guess lower because there are then 9 x 4 = 36 cards in a standard pack of cards that are lower than the Jack, there are 51 cards unseen, so the probability that the first mystery card is lower than a Jack can be calculated as 36/51 ≈ 70.6%. However, we should if given the opportunity swap the Jack. This is because relative to the Jack there are 6 more extreme , so better cards, 2,3,4,Q,K and A, the cards 6,7,8,9,10 are worse in comparison to the Jack and the card 5 has the same chance of guessing the next card correct as the Jack.
...
However, The app allows you to choose the first dealt card [2nd box on left] and vary the number of cards in a suit - it must be an odd number.  ") ,
...
When to swap is quite intuitive as explained in the introduction. It is interesting however, that if you were to swap your initial card regardless of its value (a bad idea if you want to win), the probability of winning is the same no matter what value the initial card is, with one exception, the middle card, this has an oh so very slight increase in the probability of winning relative to all the other cards. Observer this by comparing the red probabilities quoted above whilst varying the first card dealt. The simulation is not sensitive enough to reveal this, unless I presume you can perfrom a massive number of simulations. Have fun.") ,
...
```{r}
 ...
 # h5(htmlOutput("text1"))      ## print text1
 # 
 #   #-------------
 # 
 ...
 #    print(paste0(                                                               # text
 #    
 #    "Simulation is approximation to the analytical result but we do
 #     it as a programming exercise. If we swap a the card of value "
 #                 
 #      , tags$span(style="color:blue;font-weight:bold;", input$swap) ,
 #    
 #    " in a pack of "
 #    
 #      , tags$span(style="color:blue;font-weight:bold;", input$Decksize) ,
 #    
 #    " in each suit for another random card the probability of winning is approximated to be "
 #    
 #      , tags$span(style="color:blue ;font-weight:bold;", calc.prob(result)) ,
 #                   "
 #      "))
 # 
 # })
...
h5(htmlOutput("text4")) 
 #-------------
  ...
  output$text2 <- renderText({  # create interactive text t1
    ...
    " cards of each suit and play optimally (to maximise our chance of winning) is ")
...
#--------------------------
 ...
 output$text3a <- renderText({  # create interactive text t1
     ...
     result<- replicate(input$sims, simi(card1=input$swap, n=input$Decksize))   # R code   # R code
